...
export REAL_CC=$(eval echo $CC)
export CLANG_TRIPLE=$(eval echo $CLANG_TRIPLE)
export CROSS_COMPILE=$(eval echo $CROSS_COMPILE)
OUT_DIR=$(eval echo $OUT_DIR)
KDIR=$(pwd)
export CONFIG_SECTION_MISMATCH_WARN_ONLY=y
KERNEL_ENV="DTC_EXT=$KDIR/tools/dtc CONFIG_BUILD_ARM64_DT_OVERLAY=y"
	...
	case $action in
		1)
			...
			if [ ! -f "$KDIR/$OUT_DIR/.config" ]; then
				...
				make -s -C "$KDIR" O="$OUT_DIR" $KERNEL_ENV CONFIG_SECTION_MISMATCH_WARN_ONLY=y -j"$(nproc)"
				ret=$?
				if [ $ret -eq 0 ]; then
					echo -e "$BOLDGREEN[+] Kernel Building Succeed$ENDCOLOR"
					make -C "$KDIR" O="$OUT_DIR" $KERNEL_ENV INSTALL_MOD_PATH="../out_modules" CONFIG_SECTION_MISMATCH_WARN_ONLY=y modules_install -j"$(nproc)"
					ret=$?
					if [ $ret -eq 0 ]; then
		...
		2)
			...
			if [ ! -f "$KDIR/$OUT_DIR/.config" ]; then
				...
				echo -e "$BOLDGREEN[+] .config found$ENDCOLOR"
				make -s -C "$KDIR" O="$OUT_DIR" $KERNEL_ENV CONFIG_SECTION_MISMATCH_WARN_ONLY=y modules -j"$(nproc)"
				ret=$?
				if [ $ret -eq 0 ]; then
		...
		3)
			...
			if [ ! -f "$KDIR/$OUT_DIR/.config" ]; then
				...
				echo -e "$BOLDGREEN[+] .config found$ENDCOLOR"
				make -s -C "$KDIR" O="$OUT_DIR" $KERNEL_ENV CONFIG_SECTION_MISMATCH_WARN_ONLY=y modules_prepare -j"$(nproc)"
				ret=$?
				if [ $ret -eq 0 ]; then
		...
		5)
			...
			rm -rf out_modules
			make -s -C "$KDIR" O="$OUT_DIR" $KERNEL_ENV CONFIG_SECTION_MISMATCH_WARN_ONLY=y clean -j"$(nproc)" && make -s -C "$KDIR" O="$OUT_DIR" $KERNEL_ENV CONFIG_SECTION_MISMATCH_WARN_ONLY=y mrproper -j"$(nproc)"
			ret=$?
			if [ $ret -eq 0 ]; then
		...
		6)
			...
			if [ ! -f "$KDIR/arch/$ARCH/configs/$config" ]; then
				...
				make -C "$KDIR" O="$OUT_DIR" $KERNEL_ENV CONFIG_SECTION_MISMATCH_WARN_ONLY=y $config -j"$(nproc)"
				ret=$?
				if [ $ret -eq 0 ]; then
		...
		7)
			...
			make -C "$KDIR" O="$OUT_DIR" $KERNEL_ENV CONFIG_SECTION_MISMATCH_WARN_ONLY=y menuconfig -j"$(nproc)"
			;;
		8)
			echo -e "$BOLDGREEN[+] Editing Config (NCONFIG)$ENDCOLOR"
			make -C "$KDIR" O="$OUT_DIR" $KERNEL_ENV CONFIG_SECTION_MISMATCH_WARN_ONLY=y nconfig -j"$(nproc)"
			;;
		9)
